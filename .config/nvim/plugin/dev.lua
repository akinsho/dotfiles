...
function M.auto_reload(plugin)
  if _G._require == nil then
    ...
    _G.require = function(path)
      ...
      if string.find(path, module) ~= nil then require('plenary.reload').reload_module(path) end
  ...
  as.augroup('DevReload', {
    {
      ...
      command = function()
        ...
        if not cwd:match(vim.env.PROJECTS_DIR) then return end
        local lua_dir = cwd .. '/lua/'
        if fn.isdirectory(lua_dir) == 0 then return end
        local names = require('pl.dir').getdirectories(lua_dir)
        if not names[1] then return end
        local name = fn.fnamemodify(names[1], ':t')
        M.auto_reload(name)
        vim.schedule(function() vim.notify(fmt('autoreloading %s', name)) end)
      end,
    },
  })
end
